...
class TestTokenizerMapper(unittest.TestCase):
    ...
    def test_map(self):
        """Test that TokenizationMapper returns the right fields and is doing something right"""
        ...
        assert len(new_dataset[0]['attention_mask']) == \
               len(new_dataset[0]['input_ids'])    # mask same dimension as inputs
        assert self.tokenizer.decode(new_dataset[0]['input_ids'][0]) == \
               '[CLS] this is a sentence. [SEP]'
        assert self.tokenizer.decode(new_dataset[0]['input_ids'][1]) == \
    ...
    def test_truncation_max_length(self):
        ...
        assert self.tokenizer.decode(new_dataset[1]['input_ids'][0]) == \
               '[CLS] this is the next instance. [SEP]'

    def test_overflow(self):
        """
        offsets are start:end char spans into the original text. each wordpiece has
        its own start/end.
        ...
        assert self.tokenizer.decode(new_dataset[0]['input_ids'][2]) == \
               '[CLS]. [SEP]'
        assert self.tokenizer.decode(new_dataset[0]['input_ids'][3]) == \
               '[CLS] this is the subsequent unit in this instance [SEP]'
        assert self.tokenizer.decode(new_dataset[0]['input_ids'][4]) == \
    ...
    def test_char_offsets(self):
        ...
        new_dataset = mapper.map(dataset)
        assert [dataset[0]['text'][0][start:end]
    ...
    def test_split_into_words(self):
        ...
        In part 1:
            compare this with `test_char_offsets()`. there, we see `offset_mapping`
            has list elements,
            but here, it's collapsed into one field

        In part 2:
            when `is_split_into_words=False`, `input_ids` keeps elements in the List[str]
            separate. that is, tokenizes each separately.
            when `is_split_into_words=True`, `input_ids` collapses the List[str] into a
            single str for tokenization.
            to see difference, compare with the test results from `test_map()`

        In part 3:
            when we set `return_overflowing_tokens=True`, we gain the list elements
            of `input_ids` again.
            what happens here is, the list elements are stitched together due to
            ...
            specifically, the boundary between `...ten word pieces.` and `This is
            the subsequent...` is gone now.
        """
        mapper = TokenizerMapper(
            input_field='text',
            tokenizer=self.tokenizer,
        ...
        new_dataset = mapper.map(dataset)
        assert self.tokenizer.decode(new_dataset[0]['input_ids']) == \
    ...
    def test_return_words(self):
        ...
        first, word pieces are correctly mapped. see how Pterodactyl, which is split
        into 5 wordpieces, is correctly mapped back to its original word
        second, despite truncation & returning overflow causing there to be additional
        new sequences, that we can still map back to
         the original word in the sequence. See how tokens in sequence [1] and [2] can
         still get mapped back to the original word.
        """
        mapper = TokenizerMapper(
            input_field='text',
            tokenizer=self.tokenizer,
        ...
        assert new_dataset[0]['words'][0] == \
               [None, 'This', 'is', 'a', 'Pterodactyl', 'Pterodactyl', 'Pterodactyl',
                'Pterodactyl', 'Pterodactyl', None]
        assert new_dataset[0]['words'][1] == \
               [None, 'that', 'will', 'be', 'truncated', 'because', 'it', 'is', 'longer', None]
        assert new_dataset[0]['words'][2] == \
               [None, 'than', 'ten', 'word', 'pieces', '.', 'This', 'is', 'the', None]

...
class TestValidUnicodeMapper(unittest.TestCase):
    ...
    def test_map(self):
        ...
        mapper = ValidUnicodeMapper(
            input_fields=['tokens'],
            unicode_categories=["Cc", "Cf", "Co", "Cs", "Mn",
                                "Zl", "Zp", "Zs"],
            replace_token='[UNK]'
        )
        dataset = Dataset([
...
class TestTokenizerMapper(unittest.TestCase):
    ...
    def test_return_words(self):
        ...
        dataset = Dataset([{
            'text': [
                'This', 'is', 'a', 'Pterodactyl', 'that', 'will', 'be',
                'truncated', 'because', 'it', 'is', 'longer', 'than', 'ten',
                'word', 'pieces', '.', 'This', 'is', 'the', 'subsequent',
                'Pterodactyl', 'in', 'this', 'instance', 'that', 'will', 'be',
                'separately', 'truncated', '.'
            ]
        }])
        new_dataset = mapper.map(dataset)
        ...
        self.assertEqual(
            ...
            [None, 'This', 'is', 'a', 'Pterodactyl', 'Pterodactyl',
             'Pterodactyl', 'Pterodactyl', 'Pterodactyl', None]
        )
        self.assertEqual(
            new_dataset[0]['words'][1],
            [None, 'that', 'will', 'be', 'truncated', 'because', 'it', 'is',
             'longer', None]
        )
        self.assertEqual(
            new_dataset[0]['words'][2],
            [None, 'than', 'ten', 'word', 'pieces', '.', 'This',
             'is', 'the', None]
        )

...
from smashed.mappers.types import (
    BinarizerMapper
)
...
class TestBinarizer(TestCase):
    def _run_tests(self, dataset: Union[list, Dataset]):
        mapper = (
            BinarizerMapper(field='a', threshold=0.7) >>
            BinarizerMapper(field='b', threshold=0.7)
        )
    ...
    def test_binarizer_list_of_dict(self):
        dataset = [{"a": [0.3, .4, .8], "b": 0.9}]
        self._run_tests(dataset)


    def test_binarizer_hf_dataset(self):
        dataset = Dataset.from_dict({"a": [[0.3, .4, .8]], "b": [0.9]})
        self._run_tests(dataset)

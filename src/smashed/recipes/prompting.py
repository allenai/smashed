...
def prompting_recipe(
    ...
    Args:
        ...
        target_template (Optional[str], optional): The template for the target
            prompt; see FillEncodedPromptMapper for more details. If not
            provided, we only use templates for the source. Defaults to None.
        ...
        extra_keep_field_names (List[str] or  Dict[str, str], optional):
            The extra fields to keep. If a list is provided, we will keep the
            fields with the same name. If a dictionary is provided, we will
            keep the fields with the name specified in the dictionary.
            Defaults to None.
        extra_encode_fields (Optional[Sequence[str]], optional): The extra
            ...
            fields besides the ones indicated in the source or target template.
            Defaults to None.
    """
    ...
    pipeline = _add_truncation_and_striding(
        pipeline=pipeline,
...
def _add_truncation_and_striding(
    ...
    for field_name in fields_to_stride:
        ...
        pipeline = pipeline.chain(strider_mapper)
    ...
    if fields_to_truncate:
        ...
        pipeline = pipeline.chain(source_truncation_mapper)
